...
local_path = os.path.join(os.getcwd(), "result/SILoss_GaussData")
...
def create(dimensions, rate, hidden_dim, df, alpha, local_path):
    os.makedirs(local_path, exist_ok=True)
...
def create_task(args):
    ...
    """
    dimensions, rate, hidden_dim, df, alpha, local_path = args
    input_path = os.path.join(local_path, f"dimension{dimensions}rate{rate:.2f}df{df:.2f}alpha{alpha:.3f}hidden{hidden_dim}")
    create(dimensions, rate, hidden_dim, df, alpha, input_path)
...
def generate_tasks(local_path):
    ...
    for rate in np.linspace(0.2, 5.0, 20):
        for df in np.linspace(0.6, 0.9, 20):
            for dimensions in range(2, 6, 2):
                for hidden_dim in range(2, 20, 2):
                    for alpha in np.linspace(0.001, 0.3, 20):
                        tasks.append((dimensions, rate, hidden_dim, df, alpha, local_path))
    return tasks
