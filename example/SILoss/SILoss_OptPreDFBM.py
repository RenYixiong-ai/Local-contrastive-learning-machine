...
local_path = os.path.join(os.getcwd(), "result/SILoss_preDFBM")
...
csv_file_path = os.path.join(local_path, datatype +'_PreDFBM_3_40.csv')
...
def objective(trial):
    ...
    output_size_list = [trial.suggest_int(f'output_size_{i}', 300, 2000) for i in range(3)] 
    d_f_list = [trial.suggest_float(f'd_f_{i}', 0.00001, 10.0) for i in range(3)]
    alpha_list = [trial.suggest_float(f'alpha_{i}', 0.00001, 10.0) for i in range(3)]

    pipeline, deal_train_loader = pre_DFBM(train_loader, output_size_list, d_f_list, alpha_list, device, train_MLP=True, losstype = "fast_StrongInter")
