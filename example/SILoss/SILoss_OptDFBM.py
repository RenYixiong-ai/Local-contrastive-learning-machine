...
local_path = os.path.join(os.getcwd(), "result/SILoss_DFBM")
...
csv_file_path = os.path.join(local_path, datatype + '_hidden1000_lam0_DFBM_1_80.csv')
...
def objective(trial):
    ...
    #output_size_list = [trial.suggest_int(f'output_size_{i}', 1000, 2000) for i in range(1)] 
    output_size_list = [1000] 
    d_f_list = [trial.suggest_float(f'd_f_{i}', 0.0001, 10.0) for i in range(1)]
    alpha_list = [trial.suggest_float(f'alpha_{i}', 0.0001, 10.0) for i in range(1)]

    pipeline, deal_train_loader =DFBM(train_loader, output_size_list, d_f_list, alpha_list, device, losstype = "fast_StrongInter")
...
study.optimize(objective, n_trials=30000)  # 进行 100 次优化搜索
