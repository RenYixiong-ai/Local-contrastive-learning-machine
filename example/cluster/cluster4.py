...
local_path = os.path.join(os.getcwd(), "result/cluster/cluster4")
...
def main(beta_list, train_loader, test_loader, name):
    input_size = images_size
    Boson_list, Fermi_list, FerBos_list = [], [], []
    accuracy_list = []
    for beta in beta_list:
        Boson, Fermi, FerBos, train_loader, test_loader, accuracy = layer_train(train_loader, test_loader, beta, input_size)
        Boson_list.append(Boson)
        Fermi_list.append(Fermi)
...
for c in np.linspace(4.0, 20.0, 50):
    beta_list = [2.0, (2.0+c)/2, c]
    name = f"beta0_{beta_list[0]:.2f}_beta1_{beta_list[1]:.2f}_beta2_{beta_list[2]:.2f}"
    main(beta_list, train_loader, test_loader, name)
