...
def main(beta):
    ...
    save_data = {
        ...
        "mean_norm_list": mean_norm_list,
        "std_norm_list": std_norm_list,
        "cluster_accuracy_list": cluster_accuracy_list,
        "MLP_accuracy_list": MLP_accuracy_list
    }

    with open(os.path.join(local_path, datatype+f"beta{beta:.2f}_data_archive.pkl"), "wb") as file:
    ...
    fig, axes = plt.subplots(2, 3, figsize=(16, 10))  # 创建一个包含2行3列子图的图像
    ...
    plt.savefig(os.path.join(local_path, datatype+f"beta{beta:.2f}_fig.png"))
    plt.close()

beta_list = np.linspace(0.1, 20, 30)
