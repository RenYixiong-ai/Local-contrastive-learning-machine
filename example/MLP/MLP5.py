...
class MLP(nn.Module):
    def __init__(self, num_hidden_layers):
        ...
        # 动态生成中间层
        layers = []
        freeze_layers = [i for i in range(num_hidden_layers//4, 3*num_hidden_layers//4)]
        in_features = 512
        for i in range(num_hidden_layers):
            layers.append(nn.Linear(in_features, 512))
            layers.append(self.relu)
            nn.init.normal_(layers[-2].weight, mean=0.0, std=0.01)  # 使用正态分布初始化权重
            in_features = 512

            if i in freeze_layers:
                layers[-2].requires_grad = False  # 冻结Linear层
...
plt.grid(True)
plt.savefig(os.path.join(local_path, "MultMLP_Freeze.png"))